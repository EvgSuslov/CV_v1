...
#import Py_Rasp_IDEA     # Для интеграции программы на одноплатный ПК


database = 'C:/Users/ruiny/Desktop/Face_scanner/Database'            # Путь к папке с фотографиями-образцами
...
while True:     # покадровая бработка видеопотока с вэб-камеры
    ...
    cv2.waitKey(0)   # 0 – следующий кадр по нажатию кнопки
                     # 1 – постоянный поток с камеры

    if round(min(faceDis),3) > 0:
        print('True')
        print(round(min(faceDis),3))
    else:
        print('False')
        print(round(min(faceDis),3))
